...
已于 2026-09-23 Accepted/完成；M3 正式合同及 Schema v7 实现已于 2026-09-23 获 owner Accepted/完成；M4–M7 未激活。M2 已实现 additive Schema v6 lifecycle/admission
...
is Accepted and M2 is complete as of 2026-09-23. M3 is Accepted and complete as of 2026-09-23;
M4–M7 remain inactive. M2
...
L3.5 盲测 Holdout v2 已在算法 `f1d03cf` 冻结后完成唯一一次有效首轮；正文仍由用户在仓库外保管。
主 suite 为 20 个 case、4 个独立任务领域和 4 个共享 Memory pool，oracle 只检查代码、测试、文件或
工具行为；5298ba0 三任务兼容 arm 单独保留。主首轮 relevant recall 为 `0.0`、irrelevant
injection 为 `0.0`、scope/revision/stale-deleted leakage 为 `0`、无关行为变化为 `0`，兼容 arm
warm `3/3`；runner 为 observation-only，task success `0/20` 不是 Provider 或真实模型收益证据。
manifest 与 suite hash 见 [memory-retrieval-holdout-v2.manifest.json](docs/evidence/memory-retrieval-holdout-v2.manifest.json)，
脱敏执行记录见 [Holdout v2 record](docs/evidence/memory-retrieval-holdout-v2-2026-09-18.md)。
...
| Persistence | SQLite schema v7；原子 Runtime journal、M1 Product 映射/恢复边界、M2 lifecycle/admission、M3 frozen request/context 与 Memory audit / SQLite v7 with atomic Runtime journal, Product lineage, and Memory audit |
...
| [实施路线 / Implementation roadmap](docs/coding-agent-v1-implementation-roadmap.md) | M0–M3 已 Accepted/完成；M4–M7 inactive |
